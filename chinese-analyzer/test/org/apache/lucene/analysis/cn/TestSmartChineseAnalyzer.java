...
public class TestSmartChineseAnalyzer extends TestCase {
  ...
  public void testChineseAnalyzer() throws IOException {
    ...
    List<String> words = TestSmartChineseAnalyzer.tokenize(sentence, null, ca, CharTermAttribute.class);
    System.out.println(words);
    for(int i = 0; i < result.length; i++) {
      assertEquals(result[i], words.get(i));
    }
    sentence = "我从小就不由自主地认为自己长大以后一定得成为一个象我父亲一样的画家, 可能是父母潜移默化的影响。其实我根本不知道作为画家意味着什么，我是否喜欢，最重要的是否适合我，我是否有这个才华。其实人到中年的我还是不确定我最喜欢什么，最想做的是什么？我相信很多人和我一样有同样的烦恼。毕竟不是每个人都能成为作文里的宇航员，科学家和大教授。知道自己适合做什么，喜欢做什么，能做好什么其实是个非常困难的问题。"
    ...
    words = TestSmartChineseAnalyzer.tokenize(sentence, null, ca, CharTermAttribute.class);
    System.out.println(words);
  }
  
  static public List<String> tokenize(String input, String fieldName, Analyzer analyzer, Class<?> klass) throws IOException {
		TokenStream stream = null;
		try {
			List<String> terms = new LinkedList<String>();
			stream = analyzer.tokenStream(fieldName, new StringReader(input));
			stream.reset();
  ...
  public static void main(String[] args) throws IOException {
    new TestSmartChineseAnalyzer().testChineseAnalyzer();
  }
